...
function JustWritingEditorPage()
	...
	wp_enqueue_style('justwriting-editor-css', plugin_dir_url(__FILE__) . 'editor.css', true, $file_version );
	wp_enqueue_style('justwriting-custom-editor-css', plugin_dir_url(__FILE__) . 'just-writing-editor.' . $file_version . '.css', true, $file_version );
	wp_enqueue_style('jquery-ui-dialog', includes_url() . 'css/jquery-ui-dialog.css');
	wp_enqueue_style('dashicons');
	
	//Enqueue the javascript.
	//wp_enqueue_script('justwriting-editor-js', plugin_dir_url(__FILE__) . 'just-writing-editor.' . $file_version . '.js' );
...
<?php
	...
	if( $JustWritingUtilities->get_user_option( 'link' ) == 'on' ) 				{ $buttons['link'] = array( 'title' => __('Insert/edit link (Alt + Shift + A)', 'just-writing'), 'onclick' => "wpLink.open();", 'both' => false ); }
...
<div id="jw-editor-container" class="jw-editor-container">
	...
	<div style="padding-top: 54px; width: <?php echo $dfw_width;?>px; display: block; margin-left: auto; margin-right: auto;" id="wp-content-wrap" class="wp-core-ui wp-editor-wrap tmce-active has-dfw wp-fullscreen-wrap wp-fullscreen-active">
		...
		<?php wp_editor( $post->post_content, 'content', array('media_buttons' => true, 'tinymce' => array( 'wp_autoresize_on' => true, 'wplink' => true ) ) ); ?>
	</div>
</div>
